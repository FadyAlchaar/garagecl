...
$tajawalR       = $fontsDir . 'tajawal-Regular.ttf';
$tajawalB       = $fontsDir . 'tajawal-Bold.ttf';
$hastajawalR    = file_exists($tajawalR);
$hastajawalB    = file_exists($tajawalB);
...
// Only add tajawal font config if the regular file actually exists
if ($hastajawalR) {
    $mpdfConfig['fontDir']      = [$fontsDir];
    $mpdfConfig['fontdata']     = [
        'tajawal' => [
            'R' => 'tajawal-Regular.ttf',
            // Use Regular for Bold too if Bold file is missing
            'B' => $hastajawalB ? 'tajawal-Bold.ttf' : 'tajawal-Regular.ttf',
            'I' => $hastajawalB ? 'tajawal-Bold.ttf' : 'tajawal-Regular.ttf',
        ]
    ];
    $mpdfConfig['default_font'] = 'tajawal';
} else {
    // Fall back to mPDF's built-in DejaVu which has good Unicode coverage
    $mpdfConfig['default_font'] = 'dejavusans';
}

$mpdf = new \Mpdf\Mpdf($mpdfConfig);

// Font configured in mPDF constructor above
...
<tr>
    <td width="35%">' . $logoHtml . '<br>
        <span style="font-family:tajawal; font-size:14pt; font-weight:bold; color:#1a1a2e">' . pclean($settings['shop_name_ar'] ?? '') . '</span><br>
...
?>
<style>
    body         { font-family: tajawal, dejavusans; font-size: 10pt; color: #1a1a2e; direction: rtl; }
